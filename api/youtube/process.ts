...
async function processQueueItem(
  ...
  try {
    ...
    console.log(`[process] Extracted ${entities.length} entities, ${relationships.length} relationships`);

    // ── STEP 4: SAVE NODES ──────────────────────────────────────────────────────
    const savedNodeMap = new Map<string, string>(); // label → node id
    let nodesCreated = 0;
    ...
    for (const entity of entities) {
      ...
      if (nodeError) {
        // Duplicate or other error — try to find existing node
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        console.warn(`[process] Chunk ${i} failed:`, err);
      }
    }
...
Only include high-confidence relationships with clear evidence. Return empty array if none found.`;
        ...
        console.warn('[process] Cross-connection discovery failed:', err);
      }
    }
    ...
    // Update channel total_videos_ingested if applicable
    if (item.channel_id) {
      try {
        // Manual increment via fetch-then-update
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`[process] Item ${item.id} failed:`, err);
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const maxItems = isCron ? MAX_ITEMS_PER_BATCH : MAX_ITEMS_PROCESS_ALL;
  const supabase = getSupabase();
  ...
  try {
    ...
    let query = supabase
      ...
      .select(`
        ...
        thumbnail_url, published_at, duration_seconds, status,
    ...
    // Scope to user if user-triggered
    if (!isCron && userId) {
    ...
    // Flatten joined channel settings into each item
    ...
    for (const item of items) {
      ...
      console.log(`[process] Processing ${item.video_id} (${item.video_title ?? 'unknown'})`);
      ...
      // Update daily counter (best-effort)
      if (result.success) {
        try {
