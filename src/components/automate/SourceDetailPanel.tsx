...
function QueueItemRow({ item }: { item: QueueItemDisplay }) {
...
// ─── Extraction settings edit/display block ──────────────────────────────────
...
function SettingsBlock({ source, onSaved }: SettingsBlockProps) {
  ...
  // Reset when source changes or editing cancelled
  const resetEdit = () => {
    setEditMode(source.mode)
    setEditEmphasis(source.emphasis)
    setEditAnchorIds(source.linkedAnchors)
    setEditInstructions(source.customInstructions ?? '')
    setSaveError(null)
  }
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button type="button" onClick={handleCancel} className="font-body" style={{ fontSize: 11, color: 'var(--color-text-secondary)', background: 'transparent', border: 'none', cursor: 'pointer' }}>
              Cancel
            </button>
            <button
              type="button"
          ...
          {ANCHOR_EMPHASIS_LEVELS.map(lvl => {
            ...
            return (
              <button
                key={lvl.id}
                type="button"
                onClick={() => setEditEmphasis(lvl.id)}
                className="font-body font-semibold"
                style={{
                  flex: 1,
                  padding: '6px 8px',
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        {source.category !== 'meeting' && (
          <button
            ...
            className="font-body font-semibold"
            style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 6, border: '1px solid var(--border-subtle)', background: 'transparent', color: 'var(--color-text-secondary)', fontSize: 11, cursor: 'pointer', transition: 'all 0.15s' }}
          >
            <Edit2 size={11} /> Edit
          </button>
        )}
      ...
      <div style={{ padding: '14px 16px', borderRadius: 10, background: 'var(--color-bg-inset)', border: '1px solid var(--border-subtle)' }}>
        ...
        {/* Linked Anchors */}
        <div style={{ marginBottom: resolvedAnchors.length > 0 || source.customInstructions ? 10 : 0 }}>
          <div className="font-body" style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-text-secondary)', marginBottom: 6 }}>
            Linked Anchors
          </div>
        ...
        {source.customInstructions && (
          <div>
            ...
            <p className="font-body" style={{ fontSize: 11, color: 'var(--color-text-body)', lineHeight: 1.5, margin: 0 }}>
              {source.customInstructions}
            </p>
          </div>
        )}
      ...
      {/* Edit mode color preview — subtle visual anchor */}
...
export function SourceDetailPanel({ source, onClose, onRefetch }: SourceDetailPanelProps) {
  const { items, loading: queueLoading } = useSourceQueue(
  ...
  const queueTotal = source.queue.pending + source.queue.processing
  ...
  const handleDisconnect = async () => {
    if (!confirm(`Disconnect "${source.name}"? Historical data will be preserved.`)) return
    setActionLoading(true)
    ...
    >
      <div style={{ padding: '24px 28px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: 9, background: catColor + '1f', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <span style={{ fontSize: 18 }}>
                {source.category === 'youtube-channel' || source.category === 'youtube-playlist' ? '▶' : '📅'}
              </span>
            </div>
            <div>
              <div className="font-display" style={{ fontSize: 16, fontWeight: 700, color: 'var(--color-text-primary)', lineHeight: 1.2 }}>
                {source.name}
              </div>
              <div className="font-body" style={{ fontSize: 11, color: 'var(--color-text-secondary)', marginTop: 2 }}>
                {source.handle ?? source.channel ?? (source.category === 'meeting' ? 'Meeting Integration' : source.category === 'youtube-playlist' ? 'YouTube Playlist' : 'YouTube Channel')}
              </div>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            style={{ background: 'transparent', border: 'none', padding: '4px 6px', borderRadius: 6, cursor: 'pointer', color: 'var(--color-text-secondary)', flexShrink: 0 }}
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 28 }}>
          {[
            { label: source.category === 'meeting' ? 'Meetings Ingested' : 'Videos Ingested', value: itemsIngested },
            { label: 'In Queue', value: queueTotal },
          ].map(stat => (
            <div key={stat.label} style={{ padding: '12px 14px', borderRadius: 8, background: 'var(--color-bg-inset)', textAlign: 'center' }}>
              <div className="font-display" style={{ fontSize: 22, fontWeight: 800, color: 'var(--color-text-primary)' }}>{stat.value}</div>
              <div className="font-body" style={{ fontSize: 10, fontWeight: 600, color: 'var(--color-text-secondary)' }}>{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginBottom: 28 }}>
          <SL>
            {source.category === 'meeting' ? 'Meetings Ingested' : 'Videos Ingested'} ({ingestedItems.length})
          </SL>
          ...
          {ingestedLoading ? (
            ...
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p className="font-body" style={{ fontSize: 11, color: 'var(--color-text-placeholder)' }}>
                No {source.category === 'meeting' ? 'meetings' : 'videos'} ingested yet
              </p>
            </div>
          ) : (
            <div style={{ background: 'var(--color-bg-inset)', border: '1px solid var(--border-subtle)', borderRadius: 10, padding: '0 14px' }}>
              ...
              {ingestedItems.length > 10 && (
                <button
                  type="button"
                  onClick={() => setShowAllIngested(p => !p)}
                  className="font-body"
        ...
        {/* ── Extraction Settings (with inline edit) ────────────────── */}
        <div style={{ marginBottom: 28 }}>
          <SettingsBlock source={source} onSaved={onRefetch} />
        </div>
